...
function grid($maparay,$Division = 5)
{
    ...
    for ($k = 0; $k <= ($Division); $k++) {
        ...
        for ($i = 0; $i <= ($Division - 1); $i++) {
            ...
            $list[$k][$i][2][0] = ($maparay[0][0] - (($distance_b / $Division))) - (abs($distance_b));
            $list[$k][$i][2][1] = ($maparay[0][1] + (($distance_a / $Division) * ($i + 1)));

            $list[$k][$i][3][0] = ($maparay[0][0] - (($distance_b / $Division))) - (abs($distance_b));
